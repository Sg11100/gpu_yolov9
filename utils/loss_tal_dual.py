...
from utils.metrics import bbox_iou,bbox_iou1
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, h=None, w=None):
        ...
        # iou = bbox_iou(pred_bboxes_pos, target_bboxes_pos, xywh=False, CIoU=True, Inner=True, MDPIoU=False, feat_h=h, feat_w=w) #
        iou = bbox_iou(pred_bboxes_pos, target_bboxes_pos, xywh=False, EIoU=True, MDPIoU=False, Inner=True) #
        loss_iou = 1.0 - iou
        ...
        if self.use_dfl:
            ...
            loss_dfl = torch.tensor(0.0).to(pred_dist.device, non_blocking=True)

        return loss_iou, loss_dfl, iou

    def _df_loss(self, pred_dist, target):
        target_left = target.to(torch.long, non_blocking=True)
        target_right = target_left + 1
        weight_left = target_right.to(torch.float, non_blocking=True) - target
...
class ComputeLoss:
    ...
    def __init__(self, model, use_dfl=True):
        ...
        self.assigner2 = TaskAlignedAssigner(topk=int(os.getenv('YOLOM', 10)),
                                            ...
                                            alpha=float(os.getenv('YOLOA', 0.5)),
                                            beta=float(os.getenv('YOLOB', 6.0)))
        self.bbox_loss = BboxLoss(m.reg_max - 1, use_dfl=use_dfl).to(device, non_blocking=True)
        self.bbox_loss2 = BboxLoss(m.reg_max - 1, use_dfl=use_dfl).to(device, non_blocking=True)
        self.proj = torch.arange(m.reg_max).float().to(device, non_blocking=True)  # / 120.0
        self.use_dfl = use_dfl
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        loss[1] = self.BCEcls(pred_scores, target_scores.to(dtype, non_blocking=True)).sum() / target_scores_sum # BCE
        loss[1] *= 0.25
        loss[1] += self.BCEcls(pred_scores2, target_scores2.to(dtype, non_blocking=True)).sum() / target_scores_sum2 # BCE
...
class ComputeLossLH:
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        loss[1] = self.BCEcls(pred_scores, target_scores.to(dtype, non_blocking=True)).sum() / target_scores_sum # BCE
        loss[1] *= 0.25
        loss[1] += self.BCEcls(pred_scores2, target_scores.to(dtype, non_blocking=True)).sum() / target_scores_sum # BCE
