...
class ComputeLoss:
    ...
    def __call__(self, p, targets, masks, img=None, epoch=0):
        ...
        try:
            ...
            targets = self.preprocess(targets.to(self.device, non_blocking=True), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        ...
        loss[2] = self.BCEcls(pred_scores, target_scores.to(dtype, non_blocking=True)).sum() / target_scores_sum  # BCE
        loss[2] *= 0.25
        loss[2] += self.BCEcls(pred_scores2, target_scores2.to(dtype, non_blocking=True)).sum() / target_scores_sum2  # BCE
...
class ComputeLossLH:
    ...
    def __call__(self, p, targets, masks, img=None, epoch=0):
        ...
        loss[2] = self.BCEcls(pred_scores, target_scores.to(dtype, non_blocking=True)).sum() / target_scores_sum  # BCE
        loss[2] *= 0.25
        loss[2] += self.BCEcls(pred_scores2, target_scores.to(dtype, non_blocking=True)).sum() / target_scores_sum  # BCE
